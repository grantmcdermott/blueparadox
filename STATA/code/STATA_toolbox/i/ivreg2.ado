...
* On replay, set flag so saved eqns aren't dropped
		...
		syntax [anything(name=0)] [if] [in] [aw fw pw iw/] [, /*
			*/ FIRST FFIRST NOID NOCOLLIN SAVEFIRST SAVEFPrefix(name) SMall Robust CLuster(varlist) /*
			...
			*/ B0(string) SMATRIX(string) WMATRIX(string) sw spsd /*
...
************* Commonly used cross-products ************************

		tempname XX X1X1 X1Z ZZ Z2Z2 XZ Z1Z2 Xy Zy YY yy yyc ZY Z2y Z2Y XXinv ZZinv XPZXinv
...
* If either partial or collinearities mean some variables were dropped or changed,
* need to redo these
		if `partial_ct'>0 | "`collin'"~="" {
			mata: s_crossprods	("`lhs'",			/*
				*/				"`endo1'",			/*
				*/				"`inexog1' `ones'",	/*
				*/				"`exexog1'",		/*
...
************************************************************************************************
		...
		tempname b W S V beta lambda kclass2 j jp rss mss rmse sigmasq rankV rankS
...
* call abw code if bw() is defined and bw(auto) selected
				if `bw' != 0 {
					if `bw' == -1 {
						...
						abw `resid' `exexog1' `inexog1' `abwtouse', noconstant kernel(`kernel')
						local bw `r(abw)'
						local bwopt "bw(`bw')"
...
* Reduced form needed for AR first-stage test stat.  Also estimated if requested.
				...
				doRF "`lhs'" "`inexog1'" "`exexog1'" /*
					...
					*/ `"`clopt'"' `"`bwopt'"' `"`kernopt'"' /*
					*/ `"`saverfprefix'"' /*
					*/ "`dofminus'" "`sdofminus'" `"`sw'"' `"`spsd'"' "`ivreg2_cmd'"
...
* only when S matrix is calculated
				capture `ivreg2_cmd' `lhs' `cinexog1' /*
					...
					*/ `gmm2s' `bwopt' `kernopt' `dofmopt' `sw' `spsd' /*
...
* Should never reach here
		...
		if "`e(clustvar)'"!="" {
			...
			if "`e(kernel)'" != "" {
				local hacsubtitle4 "and kernel-robust to common correlated disturbances"
			}
		}
...
program define doFirst, rclass
	...
	foreach x of local endo {
		capture `ivreg2_cmd' `x' `inexog' `exexog' `wtexp' /*
				...
				*/ `dofmopt' `sdofmopt' `sw' `spsd' small nocollin
		if _rc ~= 0 {
...
* capture to prevent not-full-rank error warning
	capture `ivreg2_cmd' `lhs' `inexog' `exexog' `weight' if `touse', /*
		...
		*/	`robust' `clopt' `bwopt' `kernopt' `sw' `spsd' nocollin
	if _rc != 0 {
...
// deal with ts ops 
		...
		mata: s_abw("`varlist1'",`cons',"`kernel'")
...
struct ms_vcvorthog {
	...
	string scalar	sw, spsd, ivarname, tvarname, tindexname
...
printf("\n{error:Error: invalid choice of Fuller LIML parameter.}\n")
		...
		kclass = lambda - fuller/(N-cols(Z))
	}
	else {
		kclass = lambda
	}
	QXhXh=(1-kclass)*QXX + kclass*QXZ*QZZinv*QXZ'
	_makesymmetric(QXhXh)
	QXhXhinv=invsym(QXhXh)
	beta = QXy'*QXhXhinv*(1-kclass) + kclass*QZy'*QZZinv*QXZ'*QXhXhinv
...
void m_cuecrit(todo, beta, pointer py, pointer pX, struct ms_vcvorthog scalar vcvo, j, g, H)
{
	*vcvo.e[.,.] = *py - *pX * beta'
...
// y=XB => X'y=X'XB
		by = qrsolve(QX2X2,QX2y)
		e[.,.] = y-X2*by
		bZ1 = qrsolve(QX2X2, QX2Z1)
		Z1[.,.] = Z1 - X2*bZ1
	}
	else {
		e[.,.] = y
...
// And tell m_omega that ZZ is the cross-product of the partialled-out Zs
...
// in order that m_omega is passed the right ones.
...
// Having calculated AP R-sq based on non-robust AP Wald, now get robust AP Wald if needed.
		if ((robust~="") | (clustvarname~="") | (kernel~="") | (sw~="")) {
// Reset Z and ZZ so that m_omega is passed the right variables.
// vcvo.Z is a pointer so automatically updated.
...
real matrix m_omega(struct ms_vcvorthog scalar vcvo) 
{
	...
	else if ((vcvo.kernel=="Quadratic spectral") | (vcvo.kernel=="Daniell") | (vcvo.kernel=="Tent")) {
		window="spectral"
	}
	else if (vcvo.kernel~="") {
...
// Spectral windows require looping through all T-1 autocovariances
			...
			for (tau=1; tau<=TAU; tau++) {
				...
				if (kw~=0) {
					...
					tlag=st_data(., lstau+"."+vcvo.tindexname)
					tmatrix = tnow, tlag
					svar=(tnow:<.):*(tlag:<.)				// multiply column vectors of 1s and 0s to get
					tmatrix=select(tmatrix,svar)			// intersection, and replace tmatrix eliminating
															// rows where current or lagged data are missing
					if (rows(tmatrix)>0) {					// if no lags exist, tmatrix has zero rows
						wv  = (*vcvo.e)[tmatrix[.,1]] :* (*vcvo.wvar)[tmatrix[.,1]] * vcvo.wf
						wv1 = (*vcvo.e)[tmatrix[.,2]] :* (*vcvo.wvar)[tmatrix[.,2]] * vcvo.wf
						i=min(t)+vcvo.tdelta					// Loop through all possible ts (time clusters)
						while (i<=max(t)) {						// Start at t+1 (earliest t
							svar=(t[tmatrix[.,1]]:==i)
							Zsub=select((*vcvo.Z)[tmatrix[.,1],.],svar)
							Zsub1=select((*vcvo.Z)[tmatrix[.,2],.],svar)
							wvsub=select(wv,svar)
							wvsub1=select(wv1,svar)
							eZsub = quadcross(1, wvsub, Zsub)	// equivalent to colsum(wv :* Zsub)
							eZsub1= quadcross(1, wvsub1, Zsub1)	// equivalent to colsum(wv :* Zsub)
...
// to incorporate panel-varying 1/T, 1/(T-1) and 1/(T-2) as weights in summation.
		...
		for (i=1; i<=rows(info_ivar); i++) {
			...
			wvarsub=panelsubmatrix(*vcvo.wvar,i,info_ivar)
			Tsub=rows(esub)
			if (Tsub>2) {			// SW cov estimator defined only for T>2
				...
				for (j=1; j<=rows(esub); j++) {
					eZi=esub[j,1]*Zsub[j,.]
					if ((vcvo.weight=="fweight") | (vcvo.weight=="iweight")) {
						shatsub=shatsub+quadcross(eZi,eZi)*wvarsub[j]*vcvo.wf
						sigmahatsub=sigmahatsub + quadcross(esub[j,1],esub[j,1])*wvarsub[j]*vcvo.wf
						ZZsub=ZZsub+quadcross(Zsub[j,.],Zsub[j,.])*wvarsub[j]*vcvo.wf
					}
					else {
						shatsub=shatsub+quadcross(eZi,eZi)*((wvarsub[j]*vcvo.wf)^2)
						sigmahatsub=sigmahatsub + quadcross(esub[j,1],esub[j,1])*((wvarsub[j]*vcvo.wf)^2)
						ZZsub=ZZsub+quadcross(Zsub[j,.],Zsub[j,.])*((wvarsub[j]*vcvo.wf)^2)
					}
...
// Extract Eigenvector and Eigenvalues, replace EVs with abs(EVs), and reassemble shat.
// Follows Stock-Watson 2008 Econometrica, Remark 8.
	if (vcvo.spsd~="") {
