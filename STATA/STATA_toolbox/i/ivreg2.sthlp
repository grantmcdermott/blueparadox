...
{p}{cmd:ivreg2} is compatible with Stata version 10.1 or later.
...
Following Thompson (2009),
the SEs and statistics reported will be robust to disturbances
that are common to panel units and that are persistent, i.e., autocorrelated.
...
Used with 2-way clustering,
this provides SEs and statistics that are robust
...
{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s robust" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s robust}

{p 0}(GMM with user-specified first-step weighting matrix or matrix of orthogonality conditions)

{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), robust" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), robust}
...
{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s robust smatrix(S)" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s robust smatrix(S)}

{p 8 12}{stata "mat W = invsym(S)" : . mat W = invsym(S)}

{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s robust wmatrix(W)" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s robust wmatrix(W)}
...
{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), cue robust" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), cue robust}
...
{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s orthog(s)" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s orthog(s)}

{col 0}(Test the exogeneity of two excluded instruments)

{p 8 12}{stata "ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s orthog(age mrt)" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age mrt), gmm2s orthog(age mrt)}

{col 0}(Frisch-Waugh-Lovell (FWL): equivalence of estimations with and without partialling-out)

{p 8 12}{stata "ivreg2 lw s expr tenure rns _I* (iq=kww age), cluster(year)" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age), cluster(year)}

{p 8 12}{stata "ivreg2 lw s expr tenure rns _I* (iq=kww age), cluster(year) partial(_I*)" : . ivreg2 lw s expr tenure rns smsa _I* (iq=med kww age), cluster(year) partial(_I*)}

{col 0}({cmd:partial()}: efficient GMM with #clusters<#instruments feasible after partialling-out)

{p 8 12}{stata "ivreg2 lw s expr tenure rns _I* (iq=kww age), cluster(year) partial(_I*) gmm2s" : . ivreg2 lw s expr tenure rns smsa (iq=med kww age), cluster(year) partial(_I*) gmm2s}
...
{p 8 12}{stata "ivreg2 lwage exper expersq (educ=age kidslt6 kidsge6), savefirst" : . ivreg2 lwage exper expersq (educ=age kidslt6 kidsge6), savefirst}
...
{col 0}(Examples using Lage N, Large T Panel Data.  NB: T is again not very large - only
{col 0}20 - so results should be interpreted with caution)
...
{p 0 4}Angrist, J.D. and Pischke, J.-S. 2009. Mostly Harmless Ecnometrics: An Empiricist's Companion.
...
{p 0 4}Stock, J.H. and Yogo, M.  2005.  Testing for Weak Instruments in Linear IV Regression. In D.W.K. Andrews and J.H. Stock, eds. Identification and Inference for Econometric Models: Essays in Honor of Thomas Rothenberg. Cambridge: Cambridge University Press, 2005, pp. 80ñ108.
